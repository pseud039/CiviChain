...
const issues = asyncHandler(async(req,res,) => {
    ...
    const existing = {tags:tags, location:{$near:{$geometry:{type:"Point",coordinates:coords.coordinates},$maxDistance:5000}}}
...
const getAllIssues = asyncHandler( async(req,res) => {
    const allIssues = await Issue.find();
     return res.status(200).json.stringify(allIssues);
    
});

const getSortedIssues = asyncHandler( async( req,res) => {
    const { search = "", sortBy = req.query.sortBy, order="desc"}= req.query;
    const searchQuery = {
        $and:[
